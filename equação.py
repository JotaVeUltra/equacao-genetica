        ...
        while gene is None:
            if self.valor in TERMOS:
                ...
                if randint(0, 100) < 5:
                    ...
                    tronco, lado, gene = choice([self.esquerda, self.direita]).sub_arvore_aleatória()

        return tronco, lado, gene

    def get_altura(self, carry=0):
        if self.valor in TERMOS:
            return carry
        return max(self.esquerda.get_altura(carry + 1), self.direita.get_altura(carry + 1))


def gerar_equação(nó):
    if nó.valor in OPERADORES:
        equação = f'({gerar_equação(nó.esquerda)}  {nó.valor}  {gerar_equação(nó.direita)})'
    else:
        return nó.valor
    return equação
...
def cruzar(individuo_a: dict, individuo_b: dict, mutação: bool):
    cromossomo_filho_a = deepcopy(individuo_a['cromossomo'])
    cromossomo_filho_b = deepcopy(individuo_b['cromossomo'])
    ...
    return [calcular_pontuação(cromossomo_filho_a), calcular_pontuação(cromossomo_filho_b)]
...
def gerar_individuo():
    cromossomo = Nó(choice(OPERADORES))
    return calcular_pontuação(cromossomo)


def calcular_pontuação(cromossomo: Nó):
    pontuação = avaliar_adaptação(cromossomo)
    altura = cromossomo.get_altura()
    return {'cromossomo': cromossomo, 'pontuação': pontuação, 'altura': altura}
...
def avaliar_adaptação(cromossomo):
    fitness = 0
    for x, y in zip(ENTRADAS, SAIDAS):
        try:
            resultado = eval(gerar_equação(cromossomo))
...
def roleta(população, tamanho_população):
    pior = buscar_pior_adaptado(população)['pontuação']
    weights = [(abs(individuo['pontuação'] - pior) ** 5) for individuo in população]
    return choices(população, weights=weights, k=tamanho_população)


def executar(gerações, tamanho_população, taxa_mutação, cruzamento_por_geração, altura_maxima):
    ...
    for geração in range(gerações):
        ...
        if melhor_adaptado['pontuação'] == 0:
            break
        população = [individuo for individuo in nova_população if individuo['altura'] <= altura_maxima or individuo['pontuação'] == 9999999]
...
media, melhor = executar(GERAÇÕES, TAMANHO_POPULAÇÃO, TAXA_MUTAÇÃO, CRUZAMENTO_POR_GERAÇÃO, ALTURA_MAXIMA)
...
print(f'indivíduo mais adaptado: {gerar_equação(melhor["cromossomo"])}')
